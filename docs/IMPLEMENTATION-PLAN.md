...
| 2026-06-12 | UI | 🎨 Semantic theme tokens (không hardcode màu) + switch sáng/tối (mặc định sáng), chart theo theme. |
| 2026-06-12 | Feat | ✅ "Đánh theo recommend": scanner thêm ATR → đề xuất entry/SL/TP (BUY/SELL); nút Đánh (QuickTradePanel 1 lệnh) + Tạo bot (→ Trading prefilled). Trang **Orders**: theo dõi vị thế mở realtime (mark+SL/TP+PnL, bot tự cắt) + lịch sử lệnh lọc theo mode/source/status/symbol + export CSV để phân tích. /api/orders filters, /api/scan trả entry/atr/sl/tp (migration 0006). 80 pytest xanh. |
